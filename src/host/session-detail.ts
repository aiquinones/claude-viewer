...
// What the session analysis surface reads, one session at a time. No store and no cache: this is a
// couple of files opened because a row was clicked, where every other read on this surface is a scan
// over the machine that something has to keep.
...
export const requestSessionDetail = async ({
  sessionId,
  tool
}: RequestSessionDetailArgs): Promise<SessionDetail> => {
