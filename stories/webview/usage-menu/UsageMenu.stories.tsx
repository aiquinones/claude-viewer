...
interface WithUsageArgs {
  ...
  // Which layer set whichever of the three a story passes. One source for all of them: the menu
  // prints the line per setting, and a story is showing the line rather than the layering.
  source?: SettingSource;
  children: ReactNode;
}

// The menu reads the settings rather than props, so a story sets them the way the host does.
const WithUsage = ({ metric, scope, costBasis, source = 'user', children }: WithUsageArgs) => {
  const settings: ViewerSettings = {
    ...
    usage: {
      ...
      ...(costBasis ? { costBasis: { value: costBasis, source } } : {})
    }
  };
...
// Every setting the usage surface reads, in one menu: which number the figures are, which sessions
// they count, and what a Claude dollar is made of. Click to open — a hover menu closes under the
...
// All three moved off their defaults, and every group says the value is yours rather than the
// shipped one — the same provenance line the budgets card prints, for the same reason.
export const Customized: Story = {
  render: () => (
    <WithUsage metric="cost" scope="workspace" costBasis="output">
      <UsageMenu />
    </WithUsage>
  )
};
